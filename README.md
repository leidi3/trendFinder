...
# Optional: limit how many tweets are retrieved per account (max 100, default 20)
# APIFY_TWITTER_MAX_ITEMS=20
...
### Configuring Apify for X scraping

1. Create or log into your [Apify account](https://apify.com/).
2. Generate a [personal API token](https://console.apify.com/account/integrations) and set it as `APIFY_API_TOKEN`.
3. (Optional) Provide a different actor via `APIFY_TWITTER_ACTOR_ID` if you maintain your own tweet scraper.
4. (Optional) Adjust `APIFY_TWITTER_MAX_ITEMS` to cap the number of tweets fetched per account on every run (defaults to 20, hard limit of 100).
5. Re-run the application or restart your Docker container so the new environment variables take effect.
